      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-3xl md:text-4xl font-bold tracking-tight text-foreground font-mono mt-0.5">
              Command Center
            </h1>
          ...
          <form onSubmit={handleQuickAddTask} className="flex items-center space-x-2 w-full md:w-80">
            <Input
              ...
              className="font-mono font-bold text-xs h-9 bg-card/60 backdrop-blur-md"
            />
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <Card className="p-4 flex items-center justify-between">
            <div>
              <p className="text-xs font-mono font-bold uppercase text-muted-foreground">Pending Tasks</p>
          ...
          <Card className="p-4 flex items-center justify-between">
            <div>
              <p className="text-xs font-mono font-bold uppercase text-muted-foreground">Completed</p>
          ...
          <Card className="p-4 flex items-center justify-between">
            <div>
              <p className="text-xs font-mono font-bold uppercase text-muted-foreground">Active Projects</p>
              <p className="text-2xl font-bold font-mono text-foreground mt-0.5">
                {data?.counts?.active_projects ?? 0}
              </p>
            </div>
            <FolderKanban className="w-6 h-6 text-muted-foreground/60" />
          </Card>
          <Card className="p-4 flex items-center justify-between">
            <div>
              <p className="text-xs font-mono font-bold uppercase text-muted-foreground">Goals Tracked</p>
              <p className="text-2xl font-bold font-mono text-foreground mt-0.5">
          ...
          <div className="lg:col-span-2 space-y-6">
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                <div>
                  <CardTitle className="text-lg font-bold">Today's Priorities</CardTitle>
                  <CardDescription className="font-bold">Intelligently ranked by urgency, importance, and goal impact</CardDescription>
                </div>
                <Link href="/tasks">
                  <Button variant="ghost" size="sm" className="font-mono font-bold text-xs text-muted-foreground">
                    <span>View All</span>
              ...
              <CardContent className="space-y-2.5">
                {data?.top_priorities && data.top_priorities.length > 0 ? (
                      ...
                      >
                        ...
                        <div className="flex items-center space-x-2 shrink-0">
                          {task.estimated_duration && (
                            <span className="flex items-center text-xs font-mono font-bold text-muted-foreground">
                  ...
                  <p className="text-xs font-mono font-bold text-muted-foreground py-4 text-center">
            ...
            <Card className="border-primary/40 bg-primary/5 backdrop-blur-md">
              <CardHeader className="pb-2">
                <div className="flex items-center space-x-2">
                  ...
                  <CardTitle className="text-base font-bold font-mono tracking-wider uppercase">
                    AI Decision Insight
                  </CardTitle>
                </div>
              </CardHeader>
              <CardContent>
                ...
                <div className="mt-3 flex items-center justify-between">
                  ...
                  <Link href="/brief">
                    <Button variant="outline" size="sm" className="h-8 text-xs font-mono font-bold">
                      Daily Brief Details
                    </Button>
                  </Link>
                </div>
          ...
          <div className="space-y-6">
            <Card>
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                ...
                <Link href="/goals">
                  <span className="text-xs font-mono font-bold text-muted-foreground hover:text-foreground">
                    All
                  </span>
                </Link>
              </CardHeader>
              <CardContent className="space-y-3 pt-2">
                {data?.goals_summary && data.goals_summary.length > 0 ? (
                  ...
                  <p className="text-xs font-mono font-bold text-muted-foreground py-2">
              ...
              <CardContent className="space-y-2 pt-2">
                {data?.active_projects && data.active_projects.length > 0 ? (
                  data.active_projects.map((p: any) => (
                    <div
                      key={p.id}
                      className="flex items-center justify-between p-2.5 rounded-md border border-border/50 bg-card/40 text-xs font-bold"
                    >
